...
def create_datagen(top_path,
    ...
    train_file_names, validation_file_names, test_file_names = get_file_split(top_path, supercharge_crowdeds=supercharge_crowdeds)

    # Apply filters
    train_file_names = apply_file_filters(df=train_file_names,
                                          filter_hour_above=filter_hour_above,
                                          filter_category_noisy=filter_category_noisy,
                                          filter_hour_below=filter_hour_below)

    validation_file_names = apply_file_filters(df=validation_file_names,
                                               filter_hour_above=filter_hour_above,
                                               filter_category_noisy=filter_category_noisy,
                                               filter_hour_below=filter_hour_below)

    test_file_names = apply_file_filters(df=test_file_names,
                                         filter_hour_above=filter_hour_above,
                                         filter_category_noisy=filter_category_noisy,
                                         filter_hour_below=filter_hour_below)

    scale_files = pd.concat(
        [train_file_names, validation_file_names, test_file_names])
...
def get_file_split(top_path, supercharge_crowdeds=False):
    ...
    if supercharge_crowdeds:
        train = top_path + pd.read_csv(os.path.join(top_path, 'supercharged_crowdeds_train_split.csv'), header=None, squeeze=True)
    else: 

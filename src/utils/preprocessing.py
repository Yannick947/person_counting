...
class Preprocessor(object):
    ...
    def __init__(self, length_t, length_y, top_path, sample, feature_scaler, label_scaler, augmentation_factor):
    ...
    def preprocess_features(self, arr, file_name):
        ...
        arr = clean_ends(
            arr,
            del_leading=self.sample["filter_cols_upper"],
...
def apply_file_filters(df, filter_hour_above=24, filter_category_noisy=False, filter_hour_below=7):
...
def __filter_by_hour(row, filter_hour_above=0, filter_hour_below=7):
    ...
    if (hour > filter_hour_above) or (hour < filter_hour_below):
        return False
    else:
        return True

...
def add_plot_attributes(method):
    def plot_method(y_pred, y_true, mode, logdir, video_categories):
        ...
        plt.xticks(np.arange(min(np.append(y_pred, y_true)), max(np.append(y_pred, y_true))))

        plt.yticks(np.arange(min(np.append(y_pred, y_true)), max(np.append(y_pred, y_true))))
...
def visualize_input_2d(feature_frame, feature_num, timestep_num, pool_model, save_plots=False):
    ...
    fig, axs = plt.subplots(
        nrows=1, ncols=dimensions * 2, figsize=(3 * dimensions * 2, 3 * (timestep_num / feature_num) * 0.3)
    )
...
def save_confusion_matrix(y_true, y_pred, logdir, mode="Train"):
    ...
    sns.heatmap(data=mat, annot=True, linewidths=0.6)

...
def create_callbacks(logdir, hparams=None, save_best=True, reduce_on_plateau=False, max_metrics=None, schedule_step=0):
    ...
    if save_best:
        ...
        callbacks.append(
            keras.callbacks.ModelCheckpoint(
                save_path, monitor="val_mae_rescaled", save_best_only=True, mode="min", verbose=1
            )
        )
    ...
    callbacks.append(
        keras.callbacks.EarlyStopping(monitor="val_mae_rescaled", min_delta=0, patience=30, verbose=0, mode="min")
    )

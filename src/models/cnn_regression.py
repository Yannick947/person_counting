    ...
    for sample in hparams_samples:
        try:
            ...
            model = create_cnn(
                ...
                sample,
                rescale_factor,
                snap_path=args.warm_start_path,
            )
...
def train(
    ...
    max_metrics = {"epoch_mae_rescaled": "min", "epoch_acc_rescaled": "max"}

    history = model.fit_generator(
        validation_steps=int(len(datagen_validation)),
...
def create_cnn(timesteps, features, hparams, rescale_factor, snap_path=None):
    ...
    model.compile(
        loss=hparams["loss"],
        metrics=["msle", mae_rescaled, acc_rescaled],
        optimizer=optimizer,
    )
    model.summary()

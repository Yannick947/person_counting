...
# TODO: Move evaluation of run to seperate class
def evaluate_run(model, history, gen, mode, logdir, top_path, visualize=True):
    ...
    save_test_evaluation(model, feature_frames, y_true, logdir, mode, gen.batch_size, y_pred_orig, y_true_orig)
...
def save_test_evaluation(
    ...
    print("y_true_orig", y_true_orig)
    sum_y_true, sum_pred = 0, 0

    for i in range(len(y_true_orig)):
        try:
            sum_y_true += int(y_true_orig[i])
            sum_pred += float(predictions[i])
        except:
            print("Relative difference gt-pred failed for ", y_true_orig[i], predictions[i])

    additional_metrics["Sum ground truth"] = sum_y_true
    additional_metrics["Sum predictions"] = sum_pred
    additional_metrics["Relative difference gt-predictions"] = abs(sum(y_true_orig) - sum(predictions)) / len(
        y_true_orig
    )
...
def evaluate_predictions(history, y_pred_orig, y_true_orig, visualize, model, mode="test", logdir=None,
    ...
    if visualize == True:
        visualize_predictions(
            y_true=y_true_orig, y_pred=y_pred_orig, mode=mode, logdir=logdir, video_categories=video_categories
        )
...
def get_predictions(model, gen):
    ...
    return (
        np.squeeze(y_pred),
        np.squeeze(y_pred_orig),
        np.squeeze(np.array(y_true)),
        np.array(y_true_orig),
        np.array(video_type),
        feature_frames,
    )
...
def print_stats(predictions, y_true, mode):
    ...
    print(
        ...
        sum(y_true),
        "Ratio",
        abs(sum(predictions) - sum(y_true)) / len(y_true),
    )

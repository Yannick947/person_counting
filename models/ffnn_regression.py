...
def main(args=None):
    ...
    for sample in hparams_samples:
        ...
        datagen_train, datagen_validation, datagen_test = gen_cnn.create_datagen(
            args.top_path, sample, args.label_file, args.augmentation_factor
        )
        ...
        history, model = cnn_reg.train(model, datagen_train, logdir, sample, datagen_test)
        evaluate_run(
            model,
            history,
            datagen_validation,
...
def create_ffnn(timesteps, features, hparams):
    ...
    Arguments: 
    timesteps: Number of timesteps of the input data
    features: Number of columns/features of the input data 
    hparams: Set of hyperparameters for this run

    returns a feed forward neural network model
    """

    # TODO: Check compatibility with x- and y- Coordinates.
    input_layer = Input(shape=((timesteps, features, 1)))
    if hparams["pooling_type"] == "avg":
        pooling1 = AveragePooling2D(pool_size=(hparams["pool_size_x"], int(features * hparams["pool_size_y_factor"])))(
            input_layer
        )
    else:
        pooling1 = MaxPooling2D(pool_size=(hparams["pool_size_x"], int(features * hparams["pool_size_y_factor"])))(
            input_layer
        )
    ...
    output_layer = Dense(1, activation="relu", kernel_regularizer=keras.regularizers.l2(hparams["regularizer"]))(
        dense2
    )

...
from person_counting.data_generators import data_generator_cnn_classification as dg_cnn_cls
...
def main(args=None):
    ...
    for sample in hparams_samples: 
        ...
        datagen_train, datagen_test = dg_cnn_cls.create_datagen(args.top_path,
                                                             sample,
                                                             args.label_file, 
        ...
        model = create_cnn(timestep_num, feature_num, sample, rescale_factor, datagen_test.num_classes)
        ...
        evaluate_run_cls(model, history, datagen_test,  mode='test', logdir=logdir, visualize=True, top_path=args.top_path)
        evaluate_run_cls(model, history, datagen_train, mode='train', logdir=logdir, visualize=True, top_path=args.top_path)
...
def get_samples(args):
    ...
    param_grid = {
                  ...
                  'regularization'      : [i/20 for i in range(5)], 
...
def create_cnn(timesteps, features, hparams, rescale_factor, num_classes):
    ...
    Arguments: 
        ...
        rescale_factor: Factor which is used to rescale the labels
    ...
    layers.append(LSTM(units=num_classes,activation='relu', return_sequences=False, kernel_regularizer=keras.regularizers.l2(hparams['regularization']))(layers[-1]))
    layers.append(Dense(num_classes, activation='softmax', kernel_regularizer=keras.regularizers.l2(hparams['regularization']))(layers[-1]))
    
    model = Model(layers[0], layers[-1])

    optimizer = get_optimizer(hparams['optimizer'], hparams['learning_rate'])
    model.compile(loss=hparams['loss'], metrics=[f1,keras.metrics.categorical_accuracy], optimizer=optimizer)
    model.summary()
